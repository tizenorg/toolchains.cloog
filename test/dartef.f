! Generated from /home/skimo/git/cloog-parma/cloog-core/test/dartef.cloog by CLooG 0.14.0-285-g341b1cd gmp bits.
IF (n >= 1) THEN
  DO t3=n+3, 3*n+1
  ...
  END DO
  DO t1=-n+2, n-1
    DO t2=MAX(-t1+2,t1+2), MIN(-t1+4,-t1+2*n)
      DO t3=t2+2, t2+2*n
    ...
    DO t2=MAX(-t1+5,t1+3), MIN(-t1+2*n,t1+2*n)
      ...
      END DO
    END DO
    DO t2=MAX(-t1+5,-t1+2*n+1), MIN(-t1+2*n+3,t1+2*n+1)
      DO t3=1, n
        IF (MOD(t1+t2+1, 2) == 0) THEN
          S2((t1+t2-3)/2,(-t1+t2-1)/2,t3)
        END IF
